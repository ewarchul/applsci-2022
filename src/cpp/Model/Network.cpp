...
Network::Network(const Country &country, double north, double south,
                 double west, double east) {
  std::clog << "Initializing network object. It may take a while...\n";
  this->country = country;
  for (size_t i = 0; i < this->country.getCities().size(); i++) {
    if (this->country.getCities()[i].getXCoor() < north &&
        this->country.getCities()[i].getXCoor() > south &&
        this->country.getCities()[i].getYCoor() > west &&
        this->country.getCities()[i].getYCoor() < east) {
  ...
  // *****************************************    arcs_entering
  // ****************************************************
  for (size_t i = 0; i < this->getVertices().size(); i++) {
    vector<MyArc> tmp;
    for (int j = 0; j < this->getVertices()[i].getArcsEntering().size(); j++) {
      for (int k = 0; k < this->getArcs().size(); k++) {
        if (this->getArcs()[k].getId() ==
            this->getVertices()[i].getArcsEntering()[j])
  ...
  for (size_t i = 0; i < this->getVertices().size(); i++) {
    ...
    for (int j = 0; j < this->getVertices()[i].getArcsLeaving().size(); j++) {
      for (int k = 0; k < this->getArcs().size(); k++) {
        if (this->getArcs()[k].getId() ==
            this->getVertices()[i].getArcsLeaving()[j])
          tmp.emplace_back(this->getArcs()[k]);
      }
    }
  ...
  // *****************************************    streams_sizes
  // ****************************************************
  for (size_t i = 0; i < this->getRealisations().size(); i++) {
    vector<int> tmp;
  ...
  // ******************************************
  for (size_t i = 0; i < this->getRealisations().size(); i++)
    realisation_target.emplace_back(
        this->getRealisations()[i].getDestination());

  // *****************************************    travel_cost
  // ******************************************************
  for (size_t i = 0; i < this->getArcs().size(); i++) {
    vector<int> tmp;
    for (int j = 0; j < VEHICLES_NUMBER; j++) {
      tmp.emplace_back(
  ...
  for (size_t i = 0; i < this->getArcs().size(); i++) {
    ...
    for (int j = 0; j < VEHICLES_NUMBER; j++) {
      tmp.emplace_back(
          8 /*this->getArcs()[i].getLength() * time_coefficients[j] / 10*/); // porownac z writerem
    }
  ...
  // *****************************************    period_time
  // ******************************************************
  for (size_t i = 0; i < PERIODS_NUMBER; i++) {
...
void Network::generate_arcs(double coefficient, int max_vertices) {
  ...
  for (size_t i = 0; i < vertices.size(); i++) {
    ...
    for (int j = 0; j < vertices.size(); j++) {
      if (vertices[j].getXCoor() > northest_coor &&
          sortedCitiesChecklist[j] == 0) {
        northest = j;
  ...
  for (size_t i = 2; i < sortedCities.size(); i++) {
    int closest = -1;
    int min_distance = 10000;

    for (int j = 0; j < i; j++) {
      bool result = false;
      if ((MyArc::calculateLength(sortedCities[i], sortedCities[j]) <
           min_distance)) {
        for (int k = 0; k < arc_sources.size(); k++) {
          if (check_crossroads(
                  sortedCities[i], sortedCities[j],
                  sortedCities[arc_sources[k]],
                  sortedCities[arc_targets[k]])) // pamietaj o max_vertices
          {
            result = true;
            break;
          }
  ...
  for (size_t i = counterBis; i < sortedCities.size() * coefficient; i += 2) {
    srand(time(nullptr));
    int src = rand() % sortedCities.size();
    int dest = rand() % sortedCities.size();
    bool result = false;
    for (int j = 0; j < arc_targets.size(); j++) {
      result = check_crossroads(sortedCities[src], sortedCities[dest],
                                sortedCities[arc_sources[j]],
                                sortedCities[arc_targets[j]]);
      if (result)
        break;
    }
    ...
    if (sortedCities[src].getArcsLeaving().size() >= max_vertices ||
        sortedCities[dest].getArcsEntering().size() >= max_vertices) {
      i -= 2;
      continue;
    }
    ...
    if (abs(sortedCities[src].getXCoor() - sortedCities[dest].getXCoor()) >
        abs(sortedCities[src].getYCoor() - sortedCities[dest].getYCoor())) {
      i -= 2;
      continue;
    }
...
void Network::generate_realisations(int max_value, const Country &country) {
  ...
  for (size_t i = 0; i < this->vertices.size(); i++) {
    for (int j = i + 1; j < this->vertices.size(); j++) {
      ...
      this->realisations.emplace_back(counter, vertices[i], vertices[j], value,
                                      shortestPath[0]);
      counter++;
    }
  }
  ...
  //    int counter = 0;
  //    for (size_t i = 0; i < vertices.size()*(this->vertices.size()-1)/2; i++)
  //    {
  ...
  //        {
  //            if ((vertices[src].getId() ==
  //            realisations[j].getSource().getId() && vertices[dest].getId() ==
  //            realisations[j].getDestination().getId()) ||
  //                src == dest)
  //            {
  //                allowed = false;
  //                break;
  ...
  //        }
  //
  //        realisations.emplace_back(counter, vertices[src], vertices[dest],
  //        value); counter++;
  //    }
}
...
void Network::divideRealisations() {
  ...
  for (size_t i = 0;
       i < NUMBER_OF_DIVISIONS_PER_REALISATION * initialRealisationsNumber;
       i++) {
    ...
    if (chosenPath.getVerticesVisited().size() >= 4) {
      ...
      int sumOfArcs = chosenVertex.getArcsLeaving().size() +
                      chosenVertex.getArcsEntering().size();

      if (sumOfArcs > 8) {
        MyRealisation firstHalf(
            realisationToDivide, realisations[realisationToDivide].getSource(),
            country.getCities()[this->realisations[realisationToDivide]
                                    .getShortestPath()
                                    .getVerticesVisited()[cutPoint]],
            realisations[realisationToDivide].getMass(),
            vector<int>(chosenPath.getVerticesVisited().begin(),
                        chosenPath.getVerticesVisited().begin() + cutPoint +
                            1));
        MyRealisation secondHalf(
            realisations.size(),
            country.getCities()[this->realisations[realisationToDivide]
                                    .getShortestPath()
                                    .getVerticesVisited()[cutPoint]],
            realisations[realisationToDivide].getDestination(),
            realisations[realisationToDivide].getMass(),
...
bool Network::check_crossroads(const MyVertex &src, const MyVertex &dst,
                               const MyVertex &srcBis, const MyVertex &dstBis) {
  if ((src.getXCoor() == srcBis.getXCoor() &&
       src.getYCoor() == srcBis.getYCoor()) &&
      !(dst.getXCoor() == dstBis.getXCoor() &&
        dst.getYCoor() == dstBis.getYCoor()))
    return false;
  if (!(src.getXCoor() == srcBis.getXCoor() &&
        src.getYCoor() == srcBis.getYCoor()) &&
      (dst.getXCoor() == dstBis.getXCoor() &&
       dst.getYCoor() == dstBis.getYCoor()))
    return false;
  if ((src.getXCoor() == dstBis.getXCoor() &&
       src.getYCoor() == dstBis.getYCoor()) &&
      !(dst.getXCoor() == srcBis.getXCoor() &&
        dst.getYCoor() == srcBis.getYCoor()))
    return false;
  if (!(src.getXCoor() == dstBis.getXCoor() &&
        src.getYCoor() == dstBis.getYCoor()) &&
      (dst.getXCoor() == srcBis.getXCoor() &&
...
double Network::multiplication(const MyVertex &v1, const MyVertex &v2,
                               const MyVertex &v3) {
  return (v2.getXCoor() - v1.getXCoor()) * (v3.getYCoor() - v1.getYCoor()) -
         (v3.getXCoor() - v1.getXCoor()) * (v2.getYCoor() - v1.getYCoor());
}

bool Network::lies_between(const MyVertex &v1, const MyVertex &v2,
                           const MyVertex &v3) {
...
vector<MyRealisation> Network::divideRandomRealisation(int realIndex) {
  ...
  if (chosenPath.getVerticesVisited().size() >= 3) {
    ...
    //        sumOfArcs = chosenVertex.getArcsLeaving().size() +
    //        chosenVertex.getArcsEntering().size();

    MyRealisation firstHalf(
        realIndex, realisations[realIndex].getSource(),
        country.getCities()[this->realisations[realIndex]
                                .getShortestPath()
                                .getVerticesVisited()[cutPoint]],
        realisations[realIndex].getMass(),
        vector<int>(chosenPath.getVerticesVisited().begin(),
    ...
    MyRealisation secondHalf(
        ...
        country.getCities()[this->realisations[realIndex]
                                .getShortestPath()
                                .getVerticesVisited()[cutPoint]],
        realisations[realIndex].getDestination(),
        realisations[realIndex].getMass(),
        vector<int>(chosenPath.getVerticesVisited().begin() + cutPoint,
                    chosenPath.getVerticesVisited().end()));
...
const vector<MyVertex> &Network::getVertices() const { return vertices; }

const vector<MyArc> &Network::getArcs() const { return arcs; }

const vector<MyRealisation> &Network::getRealisations() const {
  return realisations;
}

const string *Network::getPeriods() const { return periods; }

const double *Network::getVehicleCoefficients() const {
  return vehicle_coefficients;
}

const double *Network::getTimeCoefficients() const { return time_coefficients; }

const string *Network::getVehicles() const { return vehicles; }

const string *Network::getDim() const { return dim; }
...
const int *Network::getCapacity() const { return capacity; }
...
const vector<int> &Network::getVertexOpen() const { return vertex_open; }

const vector<int> &Network::getVertexClosed() const { return vertex_closed; }
...
const vector<int> &Network::getPeriodTime() const { return period_time; }
