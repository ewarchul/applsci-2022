...
Path::Path(vector<int> path) { this->verticesVisited = path; }

const vector<int> &Path::getVerticesVisited() const { return verticesVisited; }
...
void Path::setLength(int length) { Path::length = length; }
...
void Path::removeVertex(int index, const vector<MyVertex> &vertices) {
  if (index == 0)
    length -= MyArc::calculateLength(vertices[verticesVisited[0]],
                                     vertices[verticesVisited[1]]);
  else if (index == verticesVisited.size() - 1)
    length -= MyArc::calculateLength(
        vertices[verticesVisited[verticesVisited.size() - 2]],
