...
MyArc::MyArc(int id, MyVertex source, MyVertex target) : id(id), source(source), target(target) {
...
int MyArc::calculateLength(MyVertex source, MyVertex target) {
    ...
    double init = (sin(drlat / 2.)) * (sin(drlat / 2.)) +
...
int MyArc::getId() const {
    return id;
}

const MyVertex &MyArc::getSource() const {
    return source;
}

const MyVertex &MyArc::getTarget() const {
    return target;
}

MyVertex MyArc::getSource2() const {
    return source;
}

MyVertex MyArc::getTarget2() const {
    return target;
}

int MyArc::getLength() const {
    return length;
}

const string &MyArc::getCode() const {
    return code;
}

void MyArc::setLength(int length) {
    MyArc::length = length;
}

MyArc::MyArc(int id, const MyVertex &source, const MyVertex &target, int length) : id(id), source(source),
...
void MyArc::change_temp_cost(int cost) {
    temp_cost += cost;
}
