...
const vector<int> &Car::getMassCapacities() const {
    return massCapacities;
}
...
const vector<int> &Car::getVolumeCapacities() const {
...
vector<int> Car::findInterval(const MyRealisation &realisation) {
    ...
    for (int i = 0; i < this->path.getVerticesVisited().size(); i++) {
        if (this->path.getVerticesVisited()[i] == realisation.getSource().getId())
            indexes.push_back(i);
        if (this->path.getVerticesVisited()[i] == realisation.getDestination().getId())
            indexes.push_back(i);
    }
...
void Car::unloadRealisation(const MyRealisation &realisation) {
    for (int i = 0; i < this->realisationsTaken.size(); i++) {
...
void Car::cutPath(const Network &network) {
    ...
    while (possibleStart == true || possibleEnd == true) {
        for (int i = 0; i < realisationsTaken.size(); i++) {
            ...
            if (network.getRealisations()[realisationsTaken[i]].getDestination().getId() ==
        ...
        if (possibleEnd == true) {
            path.removeVertex(path.getVerticesVisited().size() - 1, network.getVertices());
            massCapacities.erase(massCapacities.begin() + massCapacities.size() - 1);
            volumeCapacities.erase(volumeCapacities.begin() + volumeCapacities.size() - 1);
        }
    }
}
...
void Car::divideRealisation(Network &network) {
    ...
    MyRealisation realisationToRemove = network.getRealisations()[realisationIndex];
    vector<MyRealisation> newRealisations = network.divideRandomRealisation(realisationIndex);
...
void Car::printCar(const Network &network) {
    cout << "Typ: " << type << ", liczba załadowanych realizacji: " << realisationsTaken.size()
         << " Przejechane kilometry: "
         << path.getLength() << endl;
    cout << "Poszczegolne pojemnosci: ";
    for (int i = 0; i < massCapacities.size(); i++)
    ...
    cout << "\nTrasa: ";
    for (int i = 0; i < path.getVerticesVisited().size(); i++)
...
void Car::changeTypeOfCar() {
    ...
    // jeśli to tir to sprawdzamy tylko czy może być fiatem, jeśli tak, to następny if sprawdzi go jako fiata
    if (this->type == "TIR") {
        bool check = false;
        for (int i = 0; i < this->massCapacities.size(); i++) {
    ...
    // jeśli był tir, to teraz jako fiat również zostanie sprawdzone czy nie może stać się renault
    if (this->type == "FIAT") {
        bool check = false;
        for (int i = 0; i < this->massCapacities.size(); i++) {
