...
DFS::DFS(vector<MyVertex> vertices, vector<MyArc> arcs, int size) {
    ...
    for (int i = 0; i < vertices.size(); i++) {
        ...
        for (int k = 0; k < vertices[i].getArcsLeaving().size(); k++)
            row[arcs[vertices[i].getArcsLeaving()[k]].getTarget().getId()] = arcs[vertices[i].getArcsLeaving()[k]].getTarget().getId();
...
const vector<Path> &DFS::getSolutions() const {
    return solutions;
}

void DFS::clearSolutions() {
...
void DFS::setLengths(const Country &country) {
    for (int i = 0; i < solutions.size(); i++) {
        ...
        for (int j = 0; j < solutions[i].getVerticesVisited().size() - 1; j++)
            length += MyArc::calculateLength(country.getCities()[solutions[i].getVerticesVisited()[j]],
